...
public class TestSupplierAppCSVFactDataFull 
{
	...
	public void testReadRecords() 
	{
		...
		assertTrue("Company 1 - cash and liquid assets is 10,000", 10000.0==companies.get(0).getDunnBradstreetData().getCashLiquidAssetsHistory().getCurrent().getValue().getValue());
		assertTrue("Company 1 - credit delinquency percentile is 88", 88==companies.get(0).getDunnBradstreetData().getCreditDelinquencyNationalPercentileHistory().getCurrent().getValue());
		assertTrue("Company 1 - max credit recommendation is 99.99", 99.99==companies.get(0).getDunnBradstreetData().getMaximumCreditRecommendation().getCurrent().getValue().getValue());
